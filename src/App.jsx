...
// In production (Render), React and API are on the same origin — use "".
// On Vercel → Render cross-origin, set VITE_API_URL in Vercel env vars
// to "https://nexuscompressorstore.onrender.com" (no trailing slash).
const API = import.meta.env.VITE_API_URL || "";

// WebSocket URL derives from API host automatically.
const _wsProto = typeof window !== "undefined" && window.location.protocol === "https:" ? "wss:" : "ws:";
const _wsHost  = API
  ? API.replace(/^https?:/, _wsProto)
  : (typeof window !== "undefined" ? `${_wsProto}//${window.location.host}` : "ws://localhost:5000");
const WS_URL = import.meta.env.VITE_WS_URL || `${_wsHost}/ws`;
...
async function compressClientSide(file) {
  ...
  if (SKIP_EXTS.has(ext)) return {compressed:null,skipped:true,reason:"already_compressed"};
  if (!window.ZstdCodec) return {compressed:null,skipped:true,reason:"wasm_unavailable"};
  try {
    const ab   = await file.arrayBuffer();
    const u8   = new Uint8Array(ab);
    const entr = computeEntropy(u8);
    if (entr > 7.5) return {compressed:null,skipped:true,reason:"high_entropy",entropy:entr};
    return new Promise((resolve) => {
      window.ZstdCodec.run(zstd => {
        try {
          const s    = new zstd.Simple();
          const comp = s.compress(u8, 6);
          const ratio= u8.length/comp.length;
          if (ratio < 1.05) { resolve({compressed:null,skipped:true,reason:"no_benefit",entropy:entr}); return; }
          resolve({compressed:comp,originalSize:u8.length,compressedSize:comp.length,ratio,entropy:entr,skipped:false});
        } catch(e) { resolve({compressed:null,skipped:true,reason:"error",entropy:0}); }
      });
    });
  } catch(e) { return {compressed:null,skipped:true,reason:"error"}; }
}
...
async function resumableUpload(file, token, folderId, onProgress) {
  ...
  const cr = await compressClientSide(file);
  const useComp = !cr.skipped;
  const totalSize   = useComp ? cr.originalSize : file.size;
  const dataToChunk = useComp ? cr.compressed : await file.arrayBuffer().then(b=>new Uint8Array(b));
  const totalChunks = Math.ceil(dataToChunk.length/CHUNK_SIZE);
  onProgress({stage:"compressing",pct:100,saved:useComp?totalSize-dataToChunk.length:0});

  onProgress({stage:"initialising",pct:0});
  const initRes = await ap("/upload/init",{
    method:"POST", headers:{"Content-Type":"application/json"},
    body: JSON.stringify({filename:file.name,total_size:totalSize,total_chunks:totalChunks,
      folder_id:folderId||null,pre_compressed:useComp}),
  });
  ...
  for (let i=0; i<totalChunks; i++) {
    ...
    while (attempts<3) {
      try {
        ...
        if (res.ok) break;
      } catch(e) { attempts++; if (attempts>=3) return {error:`Chunk ${i} failed after 3 retries`}; await new Promise(r=>setTimeout(r,1000*attempts)); }
      attempts++;
    }
  ...
  const finRes = await ap("/upload/finish",{
    method:"POST", headers:{"Content-Type":"application/json"},
    body: JSON.stringify({session_id,entropy:cr.entropy||null}),
  });
  return finRes.json();
}
...
async function regularUpload(file, token, folderId, onProgress) {
  ...
  const cr = await compressClientSide(file);
  const useComp = !cr.skipped;
  onProgress({stage:"uploading",pct:50,saved:useComp?(cr.originalSize-cr.compressedSize):0});
  const fd = new FormData();
  if (useComp) fd.append("file", new Blob([cr.compressed]), file.name);
  else         fd.append("file", file);
  if (folderId) fd.append("folder_id", folderId);
  const headers = {Authorization:`Bearer ${token}`};
  if (useComp) {
    headers["X-Pre-Compressed"] = "true";
    headers["X-Original-Size"]  = String(cr.originalSize);
    headers["X-Entropy"]        = String(cr.entropy||0);
  }
    ...
    <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.8)",backdropFilter:"blur(4px)",
      ...
      <div style={{background:"#161616",border:"1px solid rgba(255,255,255,0.1)",borderRadius:16,
        ...
        {progress?.stage==="uploading"&&progress?.totalChunks>1&&(
          <p style={{margin:"0 0 10px",fontSize:12,color:"rgba(255,255,255,0.35)"}}>Chunk {progress.chunk} of {progress.totalChunks}</p>
        )}
        {saved>0&&(
          <div style={{background:"rgba(16,185,129,0.08)",border:"0.5px solid rgba(16,185,129,0.2)",borderRadius:8,padding:"8px 12px",fontSize:12,color:"#10B981"}}>
            🗜 Client compression saved {fmt(saved)} before upload
          </div>
        )}
      </div>
    </div>
...
function AuthPage({onAuth}) {
  ...
  const inp = {width:"100%",padding:"11px 14px",boxSizing:"border-box",background:"rgba(255,255,255,0.06)",
    border:"1px solid rgba(255,255,255,0.12)",borderRadius:10,color:"#fff",fontSize:14,outline:"none",fontFamily:"inherit"};
  const submit = async () => {
    ...
    <div style={{display:"flex",alignItems:"center",justifyContent:"center",height:"100vh",background:"#0a0a0a",fontFamily:"var(--font-sans)"}}>
      <div style={{width:400,padding:"2.5rem",background:"#161616",border:"1px solid rgba(255,255,255,0.09)",borderRadius:20,boxShadow:"0 32px 80px rgba(0,0,0,0.6)"}}>
        ...
        <p style={{margin:"18px 0 0",textAlign:"center",fontSize:13,color:"rgba(255,255,255,0.4)"}}>
          ...
          <span onClick={()=>{setMode(mode==="login"?"signup":"login");setError("");setSuccess("");}} style={{color:"#60A5FA",cursor:"pointer",fontWeight:500}}>
    ...
    if (Object.keys(chunks).length>0&&Object.keys(chunks).length>=chunkCount) {
      ...
      fetch(`${API}/download/${fileId}`,{headers:{Authorization:`Bearer ${token}`}})
        .then(r=>r.blob()).then(blob=>{
          ...
          a.href=url; a.download=file.filename; a.click(); setTimeout(()=>URL.revokeObjectURL(url),10000);
        });
    ...
    <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.85)",backdropFilter:"blur(6px)",display:"flex",alignItems:"center",justifyContent:"center",zIndex:300}} onClick={onClose}>
      <div onClick={e=>e.stopPropagation()} style={{background:"#161616",border:"1px solid rgba(255,255,255,0.1)",borderRadius:16,padding:"1.5rem",width:560,maxWidth:"94vw",boxShadow:"0 32px 80px rgba(0,0,0,0.8)"}}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:16}}>
          <div>
            <h2 style={{margin:0,fontSize:16,fontWeight:500,color:"#fff"}}>{status==="done"?"✅ Download complete":"⬇️ P2P Download"}</h2>
        ...
        <div style={{marginBottom:14}}>
          ...
          <div style={{height:6,background:"rgba(255,255,255,0.07)",borderRadius:3,overflow:"hidden"}}>
            <div style={{height:"100%",borderRadius:3,width:`${Math.round(progress*100)}%`,background:status==="done"?"#10B981":"linear-gradient(90deg,#8B5CF6,#3B82F6)",transition:"width 0.1s"}}/>
          </div>
        </div>
        {uniquePeers.length>0&&<div style={{display:"flex",flexWrap:"wrap",gap:6,marginBottom:12}}>
          {uniquePeers.map(pid=>{const s=Object.values(chunks).find(c=>c.from_peer===pid);const col=s?.from_color||"#6B7280";const cnt=Object.values(chunks).filter(c=>c.from_peer===pid).length;
            return <div key={pid} style={{display:"flex",alignItems:"center",gap:5,background:`${col}22`,border:`1px solid ${col}`,borderRadius:6,padding:"3px 9px",fontSize:11,color:col}}><div style={{width:7,height:7,borderRadius:"50%",background:col}}/>{pid}<span style={{opacity:0.55}}>{cnt} chunks</span></div>;})}
        </div>}
        <div style={{display:"flex",flexWrap:"wrap",gap:3,marginBottom:12,maxHeight:130,overflowY:"auto",padding:"10px",background:"rgba(255,255,255,0.02)",borderRadius:8,border:"0.5px solid rgba(255,255,255,0.05)"}}>
          {Array.from({length:chunkCount},(_,i)=>{const c=chunks[i];return <div key={i} style={{width:22,height:22,borderRadius:4,background:c?c.from_color:"rgba(255,255,255,0.05)",display:"flex",alignItems:"center",justifyContent:"center",fontSize:8,color:"#fff",fontWeight:600,opacity:c?1:0.3}}>{i+1}</div>;})}
        </div>
        <div style={{background:"rgba(0,0,0,0.4)",borderRadius:6,padding:"8px 10px",fontSize:11,fontFamily:"monospace",maxHeight:70,overflowY:"auto",border:"0.5px solid rgba(255,255,255,0.05)"}}>
          {log.length===0?<span style={{color:"rgba(255,255,255,0.2)"}}>waiting…</span>
            :log.map((l,i)=><div key={i} style={{color:i===0?"#fff":"rgba(255,255,255,0.3)",marginBottom:2}}><span style={{color:l.color}}>{l.peer}</span>{" → chunk "}<span style={{color:"rgba(255,255,255,0.6)"}}>{l.idx}</span>{" "}<span style={{color:"rgba(255,255,255,0.25)"}}>+{l.ms}ms</span></div>)}
        </div>
    ...
    <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.8)",backdropFilter:"blur(4px)",display:"flex",alignItems:"center",justifyContent:"center",zIndex:200}} onClick={onClose}>
      <div onClick={e=>e.stopPropagation()} style={{background:"#1a1a1a",border:"1px solid rgba(255,255,255,0.1)",borderRadius:16,padding:"1.5rem",width:460,maxWidth:"90vw",boxShadow:"0 24px 60px rgba(0,0,0,0.7)"}}>
        ...
        {isDupe
          ...
          :<div style={{background:"rgba(16,185,129,0.06)",border:"0.5px solid rgba(16,185,129,0.2)",borderRadius:8,padding:"10px 14px",marginBottom:14,fontSize:12,color:"rgba(255,255,255,0.5)",lineHeight:1.6}}><span style={{color:"#10B981",fontWeight:500}}>Pipeline:</span>{" "}{result.pre_compressed?"browser zstd → ":""}plaintext → SHA-256 → zstd → AES-256-GCM → chunk → P2P</div>}
        ...
        <div style={{background:"rgba(255,255,255,0.03)",borderRadius:8,padding:"10px 14px",fontSize:12}}>
          {[["Ratio",`${result.ratio}×`],
            ["zstd level",result.pre_compressed?"client (level 6)":`server level ${result.level}`],
...
function ShareModal({file,token,onClose}) {
  ...
  useEffect(() => {
    ...
    ap(`/share/${file.hash}/public`).then(r=>r.json()).then(d=>{
      if (d.exists&&d.token) setPublicLink({exists:true,token:d.token,public_url:d.public_url||`${window.location.origin}/p/${d.token}`});
    }).catch(()=>{});
  ...
  const createPublicLink = async () => {
    try {
      ...
      const data = await res.json();
      if (!res.ok&&res.status!==200) { setError(data.error||"Failed to create link"); return; }
      const t   = data.token;
      const url = `${window.location.origin}/p/${t}`;
      setPublicLink({exists:true,token:t,public_url:url});
  ...
  return (
    <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.8)",backdropFilter:"blur(4px)",display:"flex",alignItems:"center",justifyContent:"center",zIndex:250}} onClick={onClose}>
      <div onClick={e=>e.stopPropagation()} style={{background:"#1a1a1a",border:"1px solid rgba(255,255,255,0.1)",borderRadius:16,padding:"1.5rem",width:500,maxWidth:"90vw",boxShadow:"0 24px 60px rgba(0,0,0,0.7)",maxHeight:"85vh",overflowY:"auto"}}>
        ...
        {/* Public link section */}
        <div style={{background:"rgba(255,255,255,0.03)",borderRadius:10,padding:"12px",marginBottom:16,border:"0.5px solid rgba(255,255,255,0.07)"}}>
          <p style={{margin:"0 0 8px",fontSize:12,fontWeight:500,color:"rgba(255,255,255,0.5)",textTransform:"uppercase",letterSpacing:"0.05em"}}>Public link</p>
          {publicLink?.exists?(
            <div>
              <div style={{display:"flex",gap:8,marginBottom:8}}>
                ...
                <button onClick={()=>copyLink(publicLink.public_url||`${window.location.origin}/p/${publicLink.token}`)}
                  style={{background:copied?"rgba(16,185,129,0.2)":"rgba(59,130,246,0.12)",border:`0.5px solid ${copied?"rgba(16,185,129,0.4)":"rgba(59,130,246,0.3)"}`,borderRadius:8,padding:"6px 14px",cursor:"pointer",fontSize:12,color:copied?"#34D399":"#60A5FA",fontFamily:"inherit",whiteSpace:"nowrap"}}>
                  {copied?"✓ Copied":"Copy"}
                </button>
              </div>
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
                <p style={{margin:0,fontSize:11,color:"rgba(255,255,255,0.3)"}}>Anyone with this link can download — no login needed</p>
                <button onClick={revokePublicLink} style={{background:"rgba(239,68,68,0.1)",border:"0.5px solid rgba(239,68,68,0.3)",borderRadius:6,padding:"3px 10px",cursor:"pointer",fontSize:11,color:"#FCA5A5",fontFamily:"inherit",whiteSpace:"nowrap",marginLeft:8}}>Revoke</button>
              </div>
            </div>
          ):(
            <div style={{display:"flex",gap:8,alignItems:"center"}}>
              <p style={{margin:0,fontSize:12,color:"rgba(255,255,255,0.4)",flex:1}}>Create a link anyone can use to download this file</p>
              <button onClick={createPublicLink} style={{background:"rgba(59,130,246,0.12)",border:"0.5px solid rgba(59,130,246,0.3)",borderRadius:8,padding:"7px 14px",cursor:"pointer",fontSize:12,color:"#60A5FA",fontFamily:"inherit",whiteSpace:"nowrap"}}>Create link</button>
            </div>
          )}
        </div>

        <p style={{margin:"0 0 8px",fontSize:12,fontWeight:500,color:"rgba(255,255,255,0.5)",textTransform:"uppercase",letterSpacing:"0.05em"}}>Share with user</p>
        ...
        <div style={{display:"flex",gap:8,marginBottom:8}}>
          ...
          <input type="number" placeholder="Days" value={expiry} onChange={e=>setExpiry(e.target.value)} style={{...inp,width:80,flex:"none"}} min="1" max="365"/>
        </div>
        ...
        <p style={{margin:"12px 0 0",fontSize:11,color:"rgba(255,255,255,0.25)"}}>Recipients can download but not delete your file.</p>
      </div>
    </div>
  );
}
...
function DuplicateError({filename,onClose}) {
  return (
    <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.8)",backdropFilter:"blur(4px)",display:"flex",alignItems:"center",justifyContent:"center",zIndex:200}} onClick={onClose}>
      <div onClick={e=>e.stopPropagation()} style={{background:"#1a1a1a",border:"1px solid rgba(239,68,68,0.3)",borderRadius:16,padding:"1.5rem",width:420,maxWidth:"90vw"}}>
        <h2 style={{margin:"0 0 12px",fontSize:15,fontWeight:500,color:"#fff"}}>⚠️ File already exists</h2>
        <p style={{margin:"0 0 16px",fontSize:13,color:"#FCA5A5",lineHeight:1.6}}>You already have <strong style={{color:"#fff"}}>"{filename}"</strong> in this folder.</p>
        <button onClick={onClose} style={{width:"100%",padding:"9px",borderRadius:8,border:"none",background:"#3B82F6",color:"#fff",fontSize:13,fontWeight:500,cursor:"pointer",fontFamily:"inherit"}}>OK</button>
      </div>
    </div>
  );
}
...
function NewFolderModal({token,parentId,onCreated,onClose}) {
  ...
  const create = async () => {
    ...
    try {
      const res  = await apiFetch(token)("/folders",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({name:name.trim(),parent_id:parentId||null})});
      const data = await res.json();
      if (!res.ok) { setError(data.error||"Failed"); return; }
...
function AdminDashboard({token,onClose}) {
  ...
  useEffect(() => {
    Promise.all([ap("/admin/stats"),ap("/admin/users")])
      .then(async([sr,ur])=>{ const[s,u]=await Promise.all([sr.json(),ur.json()]); setStats(s); setUsers(Array.isArray(u)?u:[]); })
      .finally(()=>setLoading(false));
  },[]); // eslint-disable-line
  ...
  return (
    <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.9)",backdropFilter:"blur(8px)",display:"flex",alignItems:"flex-start",justifyContent:"center",zIndex:400,overflowY:"auto",padding:"2rem 1rem"}}>
      <div style={{width:"100%",maxWidth:860,background:"#161616",border:"1px solid rgba(255,255,255,0.1)",borderRadius:20,padding:"1.75rem",boxShadow:"0 32px 80px rgba(0,0,0,0.8)"}}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:20}}>
          ...
          <button onClick={onClose} style={{background:"none",border:"none",cursor:"pointer",fontSize:20,color:"rgba(255,255,255,0.4)"}}>✕</button>
        </div>
        {loading?<p style={{color:"rgba(255,255,255,0.4)",fontSize:14}}>Loading…</p>:stats&&(<>
          ...
          <div style={{background:"rgba(255,255,255,0.03)",borderRadius:12,padding:"1rem",marginBottom:20}}>
            <p style={{margin:"0 0 10px",fontSize:12,fontWeight:500,color:"rgba(255,255,255,0.4)",textTransform:"uppercase",letterSpacing:"0.05em"}}>Live P2P Network</p>
            {!stats.peers?.length
              ?<p style={{margin:0,fontSize:13,color:"rgba(255,255,255,0.3)"}}>No peers connected</p>
              :<div style={{display:"flex",flexWrap:"wrap",gap:8}}>
                {(stats.peers||[]).map(p=>{const chunks=Object.values(p.chunks||{}).reduce((a,b)=>a+b,0);
                  return <div key={p.peer_id} style={{display:"flex",alignItems:"center",gap:6,background:`${p.color}18`,border:`0.5px solid ${p.color}44`,borderRadius:8,padding:"6px 10px"}}>
          ...
          {users.length>0&&(
            <div>
              <p style={{margin:"0 0 10px",fontSize:12,fontWeight:500,color:"rgba(255,255,255,0.4)",textTransform:"uppercase",letterSpacing:"0.05em"}}>Users</p>
              <div style={{background:"rgba(255,255,255,0.02)",border:"0.5px solid rgba(255,255,255,0.07)",borderRadius:12,overflow:"hidden"}}>
                <div style={{display:"grid",gridTemplateColumns:"2fr 1fr 1fr 1fr 1fr",gap:12,padding:"8px 14px",borderBottom:"0.5px solid rgba(255,255,255,0.07)",fontSize:11,fontWeight:500,color:"rgba(255,255,255,0.3)",textTransform:"uppercase",letterSpacing:"0.05em"}}>
                  <span>User ID</span><span>Files</span><span>Stored</span><span>Quota</span><span>Dedup saved</span>
                </div>
                {users.map(u=>{const qb=u.quota_bytes||(10*1024*1024*1024);const ub=u.total_stored||0;const up=Math.round(ub/qb*100);
                  return <div key={u.user_id} style={{display:"grid",gridTemplateColumns:"2fr 1fr 1fr 1fr 1fr",gap:12,padding:"10px 14px",borderBottom:"0.5px solid rgba(255,255,255,0.05)",fontSize:13}}>
                    <span style={{color:"rgba(255,255,255,0.5)",fontSize:11,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}} title={u.user_id}>{u.user_id?.slice(0,16)}…</span>
...
function Sidebar({stats,activeView,setActiveView,user,onSignOut,trashCount,folders,currentFolderId,onFolderClick,isAdmin,onAdminOpen}) {
  ...
  return (
    <aside style={{width:215,flexShrink:0,padding:"1rem 0.75rem",borderRight:"0.5px solid rgba(255,255,255,0.07)",display:"flex",flexDirection:"column",gap:2,overflow:"hidden"}}>
      {nav.map(item=>(
        <button key={item.id} onClick={()=>setActiveView(item.id)} style={{display:"flex",alignItems:"center",justifyContent:"space-between",padding:"9px 12px",borderRadius:10,border:"none",cursor:"pointer",fontSize:13,textAlign:"left",fontFamily:"inherit",background:activeView===item.id&&!currentFolderId?"rgba(255,255,255,0.08)":"transparent",color:activeView===item.id&&!currentFolderId?"#fff":"rgba(255,255,255,0.55)",fontWeight:activeView===item.id&&!currentFolderId?500:400,transition:"background 0.1s,color 0.1s"}}>
      ...
      {folders.length>0&&activeView==="active"&&(
        <div style={{marginTop:4,paddingTop:4,borderTop:"0.5px solid rgba(255,255,255,0.06)"}}>
          {folders.map(f=>(
            <button key={f.id} onClick={()=>onFolderClick(f.id)} style={{display:"flex",alignItems:"center",gap:8,padding:"7px 12px",borderRadius:8,border:"none",cursor:"pointer",fontSize:12,textAlign:"left",fontFamily:"inherit",width:"100%",background:currentFolderId===f.id?"rgba(255,255,255,0.08)":"transparent",color:currentFolderId===f.id?"#fff":"rgba(255,255,255,0.5)",transition:"background 0.1s"}}>
              <span>📁</span><span style={{overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{f.name}</span>
            </button>))}
        </div>)}
      ...
      {isAdmin&&(<button onClick={onAdminOpen} style={{display:"flex",alignItems:"center",gap:8,padding:"8px 12px",borderRadius:10,border:"0.5px solid rgba(239,68,68,0.3)",cursor:"pointer",fontSize:12,background:"rgba(239,68,68,0.08)",color:"#FCA5A5",fontFamily:"inherit",marginTop:4}}><span>⚡</span>Admin Dashboard</button>)}
    ...
    <header style={{height:58,display:"flex",alignItems:"center",gap:12,padding:"0 1.25rem",borderBottom:"0.5px solid rgba(255,255,255,0.07)",background:"#0f0f0f",flexShrink:0}}>
        ...
        <div style={{display:"flex",alignItems:"center",gap:4,fontSize:13,color:"rgba(255,255,255,0.5)"}}>
          {breadcrumb.map((crumb,i)=>(
            <span key={crumb.id||"root"} style={{display:"flex",alignItems:"center",gap:4}}>
              ...
              <button onClick={()=>onBreadcrumbClick(crumb.id)} style={{background:"none",border:"none",cursor:i<breadcrumb.length-1?"pointer":"default",color:i===breadcrumb.length-1?"#fff":"rgba(255,255,255,0.5)",fontSize:13,fontFamily:"inherit",padding:"2px 4px",borderRadius:4}}>{crumb.name}</button>
            </span>))}
        </div>
      ):(
      ...
      <div style={{marginLeft:"auto",display:"flex",alignItems:"center",gap:10}}>
        <div style={{display:"flex",alignItems:"center",gap:6,background:"rgba(255,255,255,0.05)",border:"0.5px solid rgba(255,255,255,0.1)",borderRadius:20,padding:"5px 12px",fontSize:12,color:"rgba(255,255,255,0.6)",flexShrink:0}}>
          ...
          {wsStatus==="connected"
            ?<span><span style={{color:myColor,fontWeight:500}}>{myPeerId}</span><span style={{color:"rgba(255,255,255,0.3)"}}> · {peerCount} peer{peerCount!==1?"s":""}</span></span>
    ...
    <div onMouseEnter={()=>setHover(true)} onMouseLeave={()=>setHover(false)}
      ...
      <div style={{display:"flex",gap:4}}>
        {(view==="active"||view==="starred")&&<>
          ...
          <button onClick={()=>onShare(file)} style={{background:"rgba(16,185,129,0.1)",border:"0.5px solid rgba(16,185,129,0.3)",borderRadius:7,padding:"5px 8px",cursor:"pointer",fontSize:12,color:"#34D399",fontFamily:"inherit"}}>🔗</button>
        ...
        {view==="shared"&&(<button onClick={()=>onP2PDownload(file)} style={{background:"rgba(59,130,246,0.1)",border:"0.5px solid rgba(59,130,246,0.3)",borderRadius:7,padding:"5px 8px",cursor:"pointer",fontSize:12,color:"#60A5FA",fontFamily:"inherit"}}>⬇</button>)}
      </div>
    </div>
  );
}
    ...
    <div onMouseEnter={()=>setHover(true)} onMouseLeave={()=>setHover(false)}
      ...
      <div style={{display:"flex",alignItems:"center",gap:10,minWidth:0}}>
        ...
        <div style={{minWidth:0}}>
          ...
          <p style={{margin:"2px 0 0",fontSize:11,color:"rgba(255,255,255,0.3)"}}>Folder · {relTime(folder.created_at)}</p>
        </div>
      </div>
      <span/><span/><span/><span/>
...
function SharedWithMeView({token,onP2PDownload}) {
  ...
  if (!items.length) return <div style={{textAlign:"center",padding:"4rem 0",color:"rgba(255,255,255,0.25)",fontSize:14}}>Nothing shared with you yet</div>;
  return (
    <div style={{background:"rgba(255,255,255,0.02)",border:"0.5px solid rgba(255,255,255,0.07)",borderRadius:14,overflow:"hidden"}}>
      ...
      {items.map(item=>{
        ...
        return <div key={item.share_token} style={{display:"grid",gridTemplateColumns:"2fr 1fr 1fr 70px 1fr auto",alignItems:"center",gap:12,padding:"10px 16px",borderBottom:"0.5px solid rgba(255,255,255,0.05)",fontSize:13}}>
          ...
          <button onClick={()=>onP2PDownload({...f,hash:f.hash,chunk_count:f.chunk_count})} style={{background:"rgba(59,130,246,0.1)",border:"0.5px solid rgba(59,130,246,0.3)",borderRadius:7,padding:"5px 8px",cursor:"pointer",fontSize:12,color:"#60A5FA",fontFamily:"inherit"}}>⬇</button>
        </div>;})}
    </div>
  );
}
...
export default function App() {
  ...
  const refresh=useCallback(async()=>{
    ...
    try{
      ...
      const [f,t,s,fol]=await Promise.all([fRes.json(),tRes.json(),sRes.json(),folRes.json?folRes.json():folRes]);
  ...
  const uploadFile=async(file)=>{
    setUploading(true);setError(null);setDupError(null);setQuotaError(null);
    setUploadFilename(file.name);setUploadProgress({stage:"compressing",pct:0});
    try{
      let data;
      if(file.size>RESUMABLE_THRESHOLD){
        data=await resumableUpload(file,token,currentFolderId,(prog)=>setUploadProgress(prog));
      }else{
        data=await regularUpload(file,token,currentFolderId,(prog)=>setUploadProgress(prog));
      }
      if(data.httpStatus===409||data.error==="duplicate_filename"){setDupError({filename:file.name});return;}
      if(data.httpStatus===413||data.error==="quota_exceeded"){setQuotaError(data.message);return;}
  ...
  const handleStar=async(h,cur)=>{await ap(`/star/${h}`,{method:"PATCH"});setFiles(p=>p.map(f=>f.hash===h?{...f,starred:!cur}:f));if(activeView==="starred"&&cur)refresh();};
  ...
  const handleDelete=async(h)=>{if(!window.confirm("Permanently delete? Cannot be undone."))return;await ap(`/delete/${h}`,{method:"DELETE"});setFiles(p=>p.filter(f=>f.hash!==h));refresh();};
  const handleHaveChunks=useCallback((fid,idxs)=>{sendMsg({type:"have",file_id:fid,chunks:idxs});},[sendMsg]);
  const handleFolderDelete=async(fid)=>{if(!window.confirm("Delete folder? Files moved to root."))return;await ap(`/folders/${fid}`,{method:"DELETE"});refresh();};
  const handleFolderRename=async(fid,name)=>{if(!name.trim())return;await ap(`/folders/${fid}`,{method:"PATCH",headers:{"Content-Type":"application/json"},body:JSON.stringify({name})});refresh();};
  ...
  if(!authReady)return(<div style={{display:"flex",alignItems:"center",justifyContent:"center",height:"100vh",background:"#0a0a0a",color:"rgba(255,255,255,0.3)",fontSize:14}}>Loading…</div>);
  if(!session)return <AuthPage onAuth={s=>setSession(s)}/>;

  return(
    <div style={{display:"flex",flexDirection:"column",width:"100%",height:"100vh",background:"#0f0f0f",color:"#fff",fontFamily:"var(--font-sans)",overflow:"hidden"}}>
      ...
      <div style={{flex:1,display:"flex",overflow:"hidden"}}>
        <Sidebar stats={stats} activeView={activeView}
          setActiveView={(v)=>{setActiveView(v);setSearchQuery("");setCurrentFolderId(null);setBreadcrumb([{id:null,name:"My Files"}]);}}
        ...
        <main style={{flex:1,overflowY:"auto",padding:"1.25rem 1.5rem",minWidth:0}}>
          {error&&(
            <div style={{background:"rgba(239,68,68,0.1)",border:"0.5px solid rgba(239,68,68,0.3)",borderRadius:10,padding:"10px 14px",marginBottom:16,fontSize:13,color:"#FCA5A5"}}>
              {error}<button onClick={()=>setError(null)} style={{float:"right",background:"none",border:"none",cursor:"pointer",color:"#FCA5A5",fontSize:14}}>✕</button>
            </div>)}
          ...
          {activeView==="active"&&files.length===0&&folders.length===0&&(
            <div
              ...
              style={{border:`1.5px dashed ${dragging?"#8B5CF6":"rgba(255,255,255,0.12)"}`,borderRadius:16,padding:"5rem 2rem",textAlign:"center",cursor:"pointer",background:dragging?"rgba(139,92,246,0.05)":"transparent",transition:"all 0.15s"}}>
              <p style={{margin:"0 0 8px",fontSize:20}}>📂</p>
              <p style={{margin:"0 0 6px",fontSize:15,fontWeight:500}}>Drop files here or click Upload</p>
              <p style={{margin:0,fontSize:13,color:"rgba(255,255,255,0.35)"}}>Compressed in browser · encrypted · P2P distributed · Supabase stored</p>
            </div>)}
          ...
          {visibleFiles.length===0&&folders.length===0&&activeView!=="active"&&activeView!=="shared"&&(
            <div style={{textAlign:"center",padding:"4rem 0",color:"rgba(255,255,255,0.25)",fontSize:14}}>
              {activeView==="starred"?"No starred files — click ☆ on a file":activeView==="trash"?"Trash is empty":""}
            </div>)}
          ...
          {(visibleFiles.length>0||folders.length>0)&&activeView!=="shared"&&(
            <div
              ...
              onDragOver={e=>{if(activeView!=="trash"){e.preventDefault();setDragging(true);}}} onDragLeave={()=>setDragging(false)}
              style={{position:"relative"}}>
              {dragging&&activeView!=="trash"&&(
                <div style={{position:"absolute",inset:0,zIndex:10,background:"rgba(139,92,246,0.15)",border:"2px dashed #8B5CF6",borderRadius:16,display:"flex",alignItems:"center",justifyContent:"center"}}>
              ...
              <div style={{background:"rgba(255,255,255,0.02)",border:"0.5px solid rgba(255,255,255,0.07)",borderRadius:14,overflow:"hidden"}}>
                ...
                {folders.map(f=><FolderRow key={f.id} folder={f} onOpen={openFolder} onDelete={handleFolderDelete} onRename={handleFolderRename}/>)}
          ...
          {visibleFiles.length===0&&searchQuery&&(
            <div style={{textAlign:"center",padding:"3rem 0",color:"rgba(255,255,255,0.3)",fontSize:14}}>No files match "{searchQuery}"</div>)}
        </main>
      </div>
