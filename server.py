"""
Nexus Backend Core – Phase 9 (Sprint 1 + Sprint 2 Integration)
============================================================
Sprint 1: Global Search parameter routing, Bulk Trashing, Drag-and-Drop.
Sprint 2: Dynamic File Versioning, Email-to-Email Sharing, Activity Auditing.
"""
...
print(f"[nexus] Sprint 2 Node Infrastructure Online → {SUPABASE_URL}")
...
def verify_token(token: str):
    if SUPABASE_JWT_SECRET:
        try:
            return pyjwt.decode(token, SUPABASE_JWT_SECRET, algorithms=["HS256"], options={"verify_aud": False})
        except Exception: pass
...
# ── Sprint 2: Activity Auditing Helper ────────────────────────────────────────
...
def log_activity(uid, action_type, filename, size=0, destination=None):
    try:
        ...
        print(f"[audit-log-error] Failed to write activity metadata: {e}")
...
def db_upsert_file(uid, entry):
    ...
    # Standard configuration insert context
...
def storage_upload(bucket, path, data):
    sb.storage.from_(bucket).upload(path, data, file_options={"content-type": "application/octet-stream", "upsert": "true"})
...
def split_upload_chunks(data, fh, uid):
    ...
    return chunks
def fetch_chunk_bytes(uid, fh, idx): return storage_download(CHUNK_BUCKET, chunk_path(uid, fh, idx))
...
# ── Feature 1: Process and store (Sprint 2 Upgraded Versioning Logic) ─────────
...
def _process_and_store(uid, filename, original_data, folder_id=None,
    ...
    # Sprint 2 Check: Compute version metadata increments if filename conflict is live
    ...
    # Audit tracking transaction log
    ...
    return {
        "status": "uploaded", "file_id": file_hash, "hash": file_hash, "filename": filename,
        "category": category, "original_size": original_size, "stored_size": stored_size,
        "ratio": round(best_ratio, 3), "level": chosen_level, "chunks": chunks, "chunk_count": chunk_count,
        "savings": original_size - stored_size, "encrypted": True, "version_number": next_version
    }

# ── Peer Registry & WebSockets ────────────────────────────────────────────────

peers = {}; peers_lock = threading.Lock()
def peer_summary():
    with peers_lock:
        return [{"peer_id": pid, "color": p["color"], "joined": p["joined"], "chunks": {fid: len(idxs) for fid, idxs in p["chunks"].items()}} for pid, p in peers.items()]
...
@app.route("/health")
def health(): return jsonify({"status": "ok", "sprint": 2})
...
def upload():
    ...
    result = _process_and_store(g.uid, file.filename, original_data, request.form.get("folder_id"))
...
def delete_file(file_id):
    ...
    log_activity(g.uid, "DELETE", row["filename"])
...
def move_file(file_hash):
    ...
    if target_folder in ["", "null"]: target_folder = None
...
def share_file(file_hash):
    ...
    if not recipient: return jsonify({"error": "Recipient context destination email missing"}), 400
    if recipient == g.email.lower(): return jsonify({"error": "Cannot self-share loop structural boundaries"}), 400
    ...
    sb.table("shared_files").insert({
        "file_id": file_row["id"], "owner_id": g.uid, "shared_with": recipient, "share_token": token
    }).execute()
...
def handle_folders():
    ...
    res = sb.table("folders").select("*").eq("user_id", g.uid).is_("parent_id", "null" if not request.args.get("parent_id") else request.args.get("parent_id")).execute()
    return jsonify(res.data or [])

@app.route("/folders/<folder_id>/breadcrumb", methods=["GET"])
@require_auth
def folder_breadcrumb(folder_id): return jsonify([{"id": None, "name": "My Files"}, {"id": folder_id, "name": "Folder"}])
...
def stats():
    ...
    return jsonify({
        "total_files": s.get("total_files", 0), "total_original": s.get("total_original", 0), "total_stored": s.get("total_stored", 0),
        "quota_bytes": s.get("quota_bytes", QUOTA_BYTES), "dynamic_quota_bonus": s.get("dynamic_quota_bonus", 0),
        "balance_usd": float(s.get("balance_usd", 0.0)), "current_plan": s.get("current_plan", "Option_A_Eco")
    })
...
def serve_react(path):
    if any(("/" + path).startswith(p) for p in API_PREFIXES): return jsonify({"error": "Route context missing"}), 404
    dist = os.path.join(os.path.dirname(__file__), "dist")
    if not os.path.exists(dist): return jsonify({"status": "online", "version": "Sprint 2"}), 200
