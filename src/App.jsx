 ...
 * ================
 * New features:
 *   - Folders: click-through navigation with breadcrumb + sidebar tree
 *   - Sharing: share file by email, view shared-with-me, revoke
 *   - Quota: 10 GB cap shown in sidebar and on upload error
 *   - Admin dashboard: platform stats, all users, live peers
 *   - Mobile: React Native uses same API (no frontend change needed)
 */
...
function P2PDownloader({ file, sendMsg, wsRef, myColor, token, onClose, onHaveChunks }) {
  ...
  const startRef = useRef(Date.now()); const assemblyRef = useRef([]);
  ...
  useEffect(() => {
    if (Object.keys(chunks).length>0&&Object.keys(chunks).length>=chunkCount) {
      ...
      fetch(`${API}/download/${fileId}`,{headers:{Authorization:`Bearer ${token}`}})
        .then(r=>r.blob()).then(blob=>{ const url=URL.createObjectURL(blob); const a=document.createElement("a"); a.href=url; a.download=file.filename; a.click(); setTimeout(()=>URL.revokeObjectURL(url),10000); });
    ...
    <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.85)",backdropFilter:"blur(6px)",display:"flex",alignItems:"center",justifyContent:"center",zIndex:300}} onClick={onClose}>
      <div onClick={e=>e.stopPropagation()} style={{background:"#161616",border:"1px solid rgba(255,255,255,0.1)",borderRadius:16,padding:"1.5rem",width:560,maxWidth:"94vw",boxShadow:"0 32px 80px rgba(0,0,0,0.8)"}}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:16}}>
          <div><h2 style={{margin:0,fontSize:16,fontWeight:500,color:"#fff"}}>{status==="done"?"✅ Download complete":"⬇️ P2P Download"}</h2>
            <p style={{margin:"3px 0 0",fontSize:12,color:"rgba(255,255,255,0.4)",overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap",maxWidth:400}}>{file.filename}</p></div>
        ...
        {uniquePeers.length>0&&<div style={{display:"flex",flexWrap:"wrap",gap:6,marginBottom:12}}>
          {uniquePeers.map(pid=>{const s=Object.values(chunks).find(c=>c.from_peer===pid);const col=s?.from_color||"#6B7280";const cnt=Object.values(chunks).filter(c=>c.from_peer===pid).length;
            return <div key={pid} style={{display:"flex",alignItems:"center",gap:5,background:`${col}22`,border:`1px solid ${col}`,borderRadius:6,padding:"3px 9px",fontSize:11,color:col}}><div style={{width:7,height:7,borderRadius:"50%",background:col}}/>{pid}<span style={{opacity:0.55}}>{cnt} chunks</span></div>;})}</div>}
...
function UploadResult({ result, onClose }) {
  const saved=result.savings||0; const p=result.original_size>0?Math.round((saved/result.original_size)*100):0; const isDupe=result.status==="deduplicated";
  return (
    <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.8)",backdropFilter:"blur(4px)",display:"flex",alignItems:"center",justifyContent:"center",zIndex:200}} onClick={onClose}>
      <div onClick={e=>e.stopPropagation()} style={{background:"#1a1a1a",border:"1px solid rgba(255,255,255,0.1)",borderRadius:16,padding:"1.5rem",width:460,maxWidth:"90vw",boxShadow:"0 24px 60px rgba(0,0,0,0.7)"}}>
        ...
        {isDupe?<div style={{background:"rgba(59,130,246,0.1)",border:"0.5px solid rgba(59,130,246,0.3)",borderRadius:8,padding:"10px 14px",marginBottom:14,fontSize:13,color:"#93C5FD"}}>Exact duplicate — <strong>{result.ref_count}× referenced</strong>. Saved {fmt(result.dedup_bytes_saved)}.</div>
          :<div style={{background:"rgba(16,185,129,0.06)",border:"0.5px solid rgba(16,185,129,0.2)",borderRadius:8,padding:"10px 14px",marginBottom:14,fontSize:12,color:"rgba(255,255,255,0.5)",lineHeight:1.6}}><span style={{color:"#10B981",fontWeight:500}}>Pipeline:</span> plaintext → SHA-256 → zstd → AES-256-GCM → chunk → Supabase Storage + P2P</div>}
        ...
        <div style={{background:"rgba(255,255,255,0.03)",borderRadius:8,padding:"10px 14px",fontSize:12}}>
          {[["Ratio",`${result.ratio}×`],["zstd level",`level ${result.level}`],["Category",`${catIcon(result.category)} ${result.category}`],["Chunks",`${result.chunk_count||"—"} × 256 KB`],...(result.entropy!=null?[["Entropy",`${result.entropy} bits`]]:[]),...(result.ml_model_version!=null?[["ML model",`v${result.ml_model_version}${result.ml_model_version===0?" (heuristic)":""}`]]:[])].map(([l,v])=>(
            <div key={l} style={{display:"flex",justifyContent:"space-between",marginBottom:5}}><span style={{color:"rgba(255,255,255,0.4)"}}>{l}</span><span style={{color:"rgba(255,255,255,0.8)",fontWeight:500}}>{v}</span></div>
          ))}
        </div>
      </div>
    </div>
    ...
    <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.8)",backdropFilter:"blur(4px)",display:"flex",alignItems:"center",justifyContent:"center",zIndex:250}} onClick={onClose}>
      <div onClick={e=>e.stopPropagation()} style={{background:"#1a1a1a",border:"1px solid rgba(255,255,255,0.1)",borderRadius:16,padding:"1.5rem",width:480,maxWidth:"90vw",boxShadow:"0 24px 60px rgba(0,0,0,0.7)"}}>
        ...
        <div style={{display:"flex",gap:8,marginBottom:8}}>
          ...
          <input type="number" placeholder="Days (optional)" value={expiry} onChange={e=>setExpiry(e.target.value)} style={{...inp,width:130,flex:"none"}} min="1" max="365"/>
        </div>
              ...
              <div key={s.shared_with} style={{display:"flex",alignItems:"center",justifyContent:"space-between",padding:"8px 10px",background:"rgba(255,255,255,0.03)",borderRadius:8,marginBottom:6}}>
                <div>
                  ...
                  <p style={{margin:"2px 0 0",fontSize:11,color:"rgba(255,255,255,0.3)"}}>
                    shared {relTime(s.created_at)}{s.expires_at?` · expires ${relTime(s.expires_at)}`:" · no expiry"}
                  </p>
                </div>
        ...
        <p style={{margin:"12px 0 0",fontSize:11,color:"rgba(255,255,255,0.25)"}}>
          Recipients get view-only access and can download but not delete your file.
        </p>
      </div>
    </div>
  );
  ...
  return (
    <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.8)",backdropFilter:"blur(4px)",display:"flex",alignItems:"center",justifyContent:"center",zIndex:200}} onClick={onClose}>
      <div onClick={e=>e.stopPropagation()} style={{background:"#1a1a1a",border:"1px solid rgba(239,68,68,0.3)",borderRadius:16,padding:"1.5rem",width:420,maxWidth:"90vw"}}>
        ...
        <p style={{margin:"0 0 16px",fontSize:13,color:"#FCA5A5",lineHeight:1.6}}>You already have a file named <strong style={{color:"#fff"}}>"{filename}"</strong> in this folder. Delete or rename it first.</p>
...
function QuotaError({ message, onClose }) {
  return (
    <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.8)",backdropFilter:"blur(4px)",display:"flex",alignItems:"center",justifyContent:"center",zIndex:200}} onClick={onClose}>
      <div onClick={e=>e.stopPropagation()} style={{background:"#1a1a1a",border:"1px solid rgba(239,68,68,0.3)",borderRadius:16,padding:"1.5rem",width:420,maxWidth:"90vw"}}>
        <h2 style={{margin:"0 0 12px",fontSize:15,fontWeight:500,color:"#fff"}}>💾 Storage quota exceeded</h2>
...
function NewFolderModal({ token, parentId, onCreated, onClose }) {
  ...
  const create = async () => {
    ...
    try {
      const res = await api(token)("/folders",{ method:"POST", headers:{"Content-Type":"application/json"},
...
function AdminDashboard({ token, onClose }) {
  ...
  const card = (label, value, color="#fff") => (
    <div style={{background:"rgba(255,255,255,0.04)",borderRadius:10,padding:"12px 14px"}}>
      <p style={{margin:0,fontSize:11,color:"rgba(255,255,255,0.4)",textTransform:"uppercase",letterSpacing:"0.05em"}}>{label}</p>
      <p style={{margin:"5px 0 0",fontSize:20,fontWeight:500,color}}>{value}</p>
    </div>
  );
      ...
      <div style={{width:"100%",maxWidth:860,background:"#161616",border:"1px solid rgba(255,255,255,0.1)",borderRadius:20,padding:"1.75rem",boxShadow:"0 32px 80px rgba(0,0,0,0.8)"}}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:20}}>
          <div>
            ...
            <p style={{margin:"3px 0 0",fontSize:12,color:"rgba(255,255,255,0.4)"}}>Platform health and user statistics</p>
          </div>
          ...
          <>
            ...
            <div style={{background:"rgba(255,255,255,0.03)",borderRadius:12,padding:"1rem",marginBottom:20}}>
              ...
              {stats.peers?.length===0
                ?<p style={{margin:0,fontSize:13,color:"rgba(255,255,255,0.3)"}}>No peers currently connected</p>
            ...
            {users.length>0&&(
              <div>
                ...
                <div style={{background:"rgba(255,255,255,0.02)",border:"0.5px solid rgba(255,255,255,0.07)",borderRadius:12,overflow:"hidden"}}>
                  ...
                  {users.map(u=>{
                    const quotaB = u.quota_bytes||QUOTA_BYTES; const usedB = u.total_stored||0;
                    const usedP = Math.round(usedB/quotaB*100);
                    return <div key={u.user_id} style={{display:"grid",gridTemplateColumns:"2fr 1fr 1fr 1fr 1fr",gap:12,padding:"10px 14px",borderBottom:"0.5px solid rgba(255,255,255,0.05)",fontSize:13}}>
                      ...
                      <span style={{color:"#F59E0B"}}>{fmt(usedB)}</span>
                      <span style={{color:usedP>80?"#FCA5A5":"#10B981"}}>{usedP}%</span>
    ...
    <aside style={{width:215,flexShrink:0,padding:"1rem 0.75rem",borderRight:"0.5px solid rgba(255,255,255,0.07)",display:"flex",flexDirection:"column",gap:2,overflow:"hidden"}}>
      ...
      <div style={{marginTop:"auto",padding:"12px",background:"rgba(255,255,255,0.03)",borderRadius:12,border:"0.5px solid rgba(255,255,255,0.06)"}}>
        ...
        <p style={{margin:0,fontSize:12,color:quotaPct>85?"#FCA5A5":"rgba(255,255,255,0.6)"}}>
          {fmt(usedBytes)} <span style={{color:"rgba(255,255,255,0.3)"}}>/ {fmt(quotaBytes)}</span>
        </p>
        {stats&&<p style={{margin:"4px 0 0",fontSize:11,color:"#10B981"}}>{fmt(totalSaved)} saved via compression</p>}
      ...
      {/* Admin button */}
      {isAdmin&&(
        <button onClick={onAdminOpen} style={{display:"flex",alignItems:"center",gap:8,padding:"8px 12px",borderRadius:10,border:"0.5px solid rgba(239,68,68,0.3)",cursor:"pointer",fontSize:12,background:"rgba(239,68,68,0.08)",color:"#FCA5A5",fontFamily:"inherit",marginTop:4,transition:"background 0.1s"}}>
  ...
  return (
    <header style={{height:58,display:"flex",alignItems:"center",gap:12,padding:"0 1.25rem",borderBottom:"0.5px solid rgba(255,255,255,0.07)",background:"#0f0f0f",flexShrink:0}}>
        ...
        <div style={{display:"flex",alignItems:"center",gap:4,fontSize:13,color:"rgba(255,255,255,0.5)"}}>
          {breadcrumb.map((crumb,i)=>(
            <span key={crumb.id||"root"} style={{display:"flex",alignItems:"center",gap:4}}>
              ...
              <button onClick={()=>onBreadcrumbClick(crumb.id)} style={{background:"none",border:"none",cursor:i<breadcrumb.length-1?"pointer":"default",color:i===breadcrumb.length-1?"#fff":"rgba(255,255,255,0.5)",fontSize:13,fontFamily:"inherit",padding:"2px 4px",borderRadius:4}}>
                {crumb.name}
              </button>
            </span>
          ))}
      ...
      <div style={{marginLeft:"auto",display:"flex",alignItems:"center",gap:10}}>
        ...
        {activeView==="active"&&(
          <button onClick={onNewFolder} style={{background:"rgba(255,255,255,0.07)",border:"0.5px solid rgba(255,255,255,0.12)",borderRadius:10,padding:"7px 14px",color:"rgba(255,255,255,0.7)",fontSize:13,cursor:"pointer",fontFamily:"inherit"}}>📁 New folder</button>
        )}
        {activeView!=="trash"&&(
          <button onClick={onUpload} disabled={uploading} style={{background:uploading?"rgba(59,130,246,0.3)":"#3B82F6",border:"none",borderRadius:10,padding:"8px 18px",color:"#fff",fontSize:13,fontWeight:500,cursor:uploading?"not-allowed":"pointer",fontFamily:"inherit",transition:"background 0.15s"}}>
...
function FolderRow({ folder, onOpen, onDelete, onRename }) {
  ...
  return (
    <div onMouseEnter={()=>setHover(true)} onMouseLeave={()=>setHover(false)}
      style={{display:"grid",gridTemplateColumns:"2fr 1fr 1fr 70px 1fr auto",alignItems:"center",gap:12,padding:"10px 16px",background:hover?"rgba(255,255,255,0.04)":"transparent",borderBottom:"0.5px solid rgba(255,255,255,0.05)",transition:"background 0.1s",fontSize:13,cursor:"pointer"}} onClick={()=>onOpen(folder.id)}>
      <div style={{display:"flex",alignItems:"center",gap:10,minWidth:0}}>
...
function SharedWithMeView({ token, onP2PDownload }) {
  ...
  const [loading,setLoading] = useState(true);
  useEffect(() => {
    api(token)("/shared-with-me").then(r=>r.json()).then(d=>{ if(Array.isArray(d)) setItems(d); }).finally(()=>setLoading(false));
  }, [token]);
...
export default function App() {
  ...
  const ap    = useMemo(()=> token ? api(token) : null, [token]);
  ...
  const handleBreadcrumbClick = (folderId) => { openFolder(folderId); };
  ...
  const uploadFile = async (file)=>{
    ...
    try {
      ...
      const res  = await ap("/upload",{method:"POST",body:fd});
      const data = await res.json();
      if (res.status===409 && data.error==="duplicate_filename") { setDupError({filename:file.name}); return; }
      if (res.status===413 && data.error==="quota_exceeded") { setQuotaError(data.message); return; }
      if (!res.ok) { setError(`Upload failed: ${data.error||res.statusText}`); return; }
  ...
  const handleFolderRename = async (fid, name) => {
    if (!name.trim()) return;
    await ap(`/folders/${fid}`,{method:"PATCH",headers:{"Content-Type":"application/json"},body:JSON.stringify({name})});
    refresh();
  };
  ...
  return (
    <div style={{display:"flex",flexDirection:"column",width:"100%",height:"100vh",background:"#0f0f0f",color:"#fff",fontFamily:"var(--font-sans)",overflow:"hidden"}}>
      ...
      <TopBar wsStatus={wsStatus} myPeerId={myPeerId} myColor={myColor} peerCount={peerCount}
        onUpload={()=>inputRef.current?.click()} onNewFolder={()=>setShowNewFolder(true)} uploading={uploading}
        searchQuery={searchQuery} setSearchQuery={setSearchQuery} activeView={activeView}
        breadcrumb={breadcrumb} onBreadcrumbClick={handleBreadcrumbClick}/>
      ...
      <div style={{flex:1,display:"flex",overflow:"hidden"}}>
        ...
        <main style={{flex:1,overflowY:"auto",padding:"1.25rem 1.5rem",minWidth:0}}>
          {error&&<div style={{background:"rgba(239,68,68,0.1)",border:"0.5px solid rgba(239,68,68,0.3)",borderRadius:10,padding:"10px 14px",marginBottom:16,fontSize:13,color:"#FCA5A5"}}>{error}<button onClick={()=>setError(null)} style={{float:"right",background:"none",border:"none",cursor:"pointer",color:"#FCA5A5",fontSize:14}}>✕</button></div>}
          ...
          {activeView==="active"&&files.length===0&&folders.length===0&&(
            <div onDrop={e=>{e.preventDefault();setDragging(false);if(e.dataTransfer.files[0])uploadFile(e.dataTransfer.files[0]);}}
              ...
              <p style={{margin:0,fontSize:13,color:"rgba(255,255,255,0.35)"}}>Compressed · encrypted · chunked · P2P distributed · Supabase stored</p>
            </div>
          )}
          ...
          {(visibleFiles.length>0||folders.length>0)&&activeView!=="shared"&&(
            <div onDrop={e=>{if(activeView!=="trash"){e.preventDefault();setDragging(false);if(e.dataTransfer.files[0])uploadFile(e.dataTransfer.files[0]);}}}
              onDragOver={e=>{if(activeView!=="trash"){e.preventDefault();setDragging(true);}}}
              onDragLeave={()=>setDragging(false)} style={{position:"relative"}}>
              {dragging&&activeView!=="trash"&&<div style={{position:"absolute",inset:0,zIndex:10,background:"rgba(139,92,246,0.15)",border:"2px dashed #8B5CF6",borderRadius:16,display:"flex",alignItems:"center",justifyContent:"center"}}><p style={{fontSize:16,fontWeight:500,color:"#A78BFA"}}>Drop to upload</p></div>}
              <div style={{background:"rgba(255,255,255,0.02)",border:"0.5px solid rgba(255,255,255,0.07)",borderRadius:14,overflow:"hidden"}}>
                <div style={{display:"grid",gridTemplateColumns:"2fr 1fr 1fr 70px 1fr auto",gap:12,padding:"8px 16px",borderBottom:"0.5px solid rgba(255,255,255,0.07)",fontSize:11,fontWeight:500,color:"rgba(255,255,255,0.3)",textTransform:"uppercase",letterSpacing:"0.05em"}}>
          ...
          {visibleFiles.length===0&&searchQuery&&<div style={{textAlign:"center",padding:"3rem 0",color:"rgba(255,255,255,0.3)",fontSize:14}}>No files match "{searchQuery}"</div>}
        </main>
      </div>
      ...
      {p2pTarget  &&<P2PDownloader file={p2pTarget} sendMsg={sendMsg} wsRef={ws} myColor={myColor}
